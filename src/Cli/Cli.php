...
use Neoan\Installer\Add\Package;
use Neoan\Installer\Creation\Creation;
use Neoan\Installer\Credentials\Credentials;
use Neoan\Installer\Helper\ServerHelper;
...
class Cli
{
    ...
    function __construct($arguments, $workPath)
    {
        ...
        $this->globalVars = json_decode(file_get_contents(dirname(dirname(__DIR__)) . '/bin/_vars.json'), true);
    }
    ...
    private function argumentConstructor($arguments)
    {
        foreach ($arguments as $arg) {
            ...
            preg_match('/^-{1,2}([a-z0-9]+):*([a-z0-9_.-]*)/i', $arg, $matches);
